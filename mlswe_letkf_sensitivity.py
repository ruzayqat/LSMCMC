"""
mlswe_letkf_sensitivity.py
===========================
Sensitivity analysis for MLSWE LETKF tuning parameters:
  - hcovlocal_scale  (localisation cutoff in km)
  - covinflate1      (RTPS inflation factor)

Runs a grid of experiments using run_mlswe_letkf_mpi.py with
--hcovlocal_scale and --covinflate1 command-line overrides,
collects mean RMSE values, and saves results incrementally to JSON.

Usage:
    python mlswe_letkf_sensitivity.py [--nprocs 5] [--config example_input_mlswe_ldata_V1.yml]
"""
...
def main():
    parser = argparse.ArgumentParser(
        description="MLSWE LETKF sensitivity analysis")
    parser.add_argument('--ncores', type=int, default=52,
                        help='Number of CPU cores for mp.Pool')
    parser.add_argument('--config', type=str,
                        default='example_input_mlswe_test100.yml',
                        help='YAML config file')
    parser.add_argument('--results', type=str,
                        default='mlswe_letkf_sensitivity_results.json',
                        help='JSON file for incremental results')
    parser.add_argument('--timeout', type=int, default=900,
                        help='Timeout per experiment in seconds (default 900)')
    parser.add_argument('--nanals', type=int, default=25,
                        help='Ensemble size (default 25)')
    args = parser.parse_args()

    # ---- Parameter grid ----
    # With K=50 ensemble members, the filter can support broader localization.
    # Grid spacing dx≈50km, dy≈56km → start from 60km (just above grid scale).
    # Finer sampling in 60–200 km range where K=25 showed transition.
    ...
    print("=" * 72)
    print("  MLSWE LETKF SENSITIVITY ANALYSIS")
    ...
    for idx, (hscale, alpha) in enumerate(experiments):
        ...
        if key in results and results[key].get("vel") is not None:
            skipped += 1
            print(f"[{idx+1:3d}/{total}] h={hscale:4d}km  α={alpha:.1f}  — SKIP (cached)")
            continue

        print(f"\n{'='*60}")
        print(f"  [{idx+1:3d}/{total}]  hscale={hscale} km,  covinflate1={alpha}")
        print(f"{'='*60}")

        cmd = [
            sys.executable, "run_mlswe_ldata_letkf_mpi.py", args.config,
        ...
        try:
            ...
            vel_m = re.search(r'Mean RMSE_vel\s*=\s*([\d.eE+\-]+)', output)
            sst_m = re.search(r'Mean RMSE_sst\s*=\s*([\d.eE+\-]+)', output)

            ssh_m = re.search(r'Mean RMSE_ssh\s*=\s*([\d.eE+\-]+)', output)
            ...
            results[key] = {"vel": vel, "sst": sst, "ssh": ssh, "time_s": round(elapsed, 1)}
            print(f"  -> RMSE_vel={vel}  RMSE_sst={sst}  RMSE_ssh={ssh}  ({elapsed:.0f}s)")
            ...
            results[key] = {"vel": None, "sst": None, "ssh": None, "time_s": round(elapsed, 1)}
        except Exception as e:
            print(f"  -> ERROR: {e}")
            results[key] = {"vel": None, "sst": None, "ssh": None, "time_s": 0}
    ...
    for i, h in enumerate(hscale_values_km):
        row = f"{h:7d}  | " + " | ".join(
            f"{vel_arr[i,j]:6.4f}" if np.isfinite(vel_arr[i,j]) else "  NaN "
            for j in range(n_a))
        print(row)
    ...
    for i, h in enumerate(hscale_values_km):
        row = f"{h:7d}  | " + " | ".join(
            f"{sst_arr[i,j]:6.3f}" if np.isfinite(sst_arr[i,j]) else "  NaN "
            for j in range(n_a))
        print(row)
    ...
    for i, h in enumerate(hscale_values_km):
        row = f"{h:7d}  | " + " | ".join(
            f"{ssh_arr[i,j]:6.3f}" if np.isfinite(ssh_arr[i,j]) else "  NaN "
            for j in range(n_a))
        print(row)
    ...
    if valid.any():
        ...
        # Also print best for each metric
        bv = np.unravel_index(np.nanargmin(vel_arr), vel_arr.shape)
        bs = np.unravel_index(np.nanargmin(sst_arr), sst_arr.shape)
        print(f"\n  BEST vel: h={hscale_values_km[bv[0]]}km α={alpha_values[bv[1]]}  "
              f"RMSE_vel={vel_arr[bv]:.5f}")
        print(f"  BEST sst: h={hscale_values_km[bs[0]]}km α={alpha_values[bs[1]]}  "
              f"RMSE_sst={sst_arr[bs]:.3f}")
