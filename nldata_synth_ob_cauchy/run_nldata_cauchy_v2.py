...
# -*- coding: utf-8 -*-
"""
run_nldata_cauchy_v2.py
=======================
NL twin experiment — V2: Per-block halo-localized MCMC with
the **correct Student-t (Cauchy, ν=1) likelihood**.
...
---------------------
    ε ~ σ · t(ν)   with ν = 1 (Cauchy)

    p(ε) = Γ((ν+1)/2) / (Γ(ν/2) √(νπ) σ) * (1 + ε²/(νσ²))^{-(ν+1)/2}

    log p(ε) = const - ((ν+1)/2) * log(1 + ε²/(νσ²))

    where  ε = y − h(Hz)  and  h(x) = arctan(x).

The V2 filter uses ``NL_SMCMC_MLSWE_Filter_V2`` (halo-localized MCMC)
and monkey-patches ``_block_mcmc_worker`` to inject the arctan
observation operator and Student-t log-likelihood.

Usage
-----
    python3 -u nldata_synth_ob_cauchy/run_nldata_cauchy_v2.py [config.yml]

Default config: ``nldata_synth_ob_cauchy/example_input_nldata_cauchy_v2.yml``
"""
import os
import sys
import numpy as np
...
import nldata_synth_ob_cauchy.run_nldata_cauchy_v1 as _v1_twin
...
def _block_mcmc_worker_studentt(args):
    ...
    ``obs_op_name`` field as ``'arctan_studentt:<nu>'``.
    """
    ...
    # Parse Student-t nu from obs_op_name  ('arctan_studentt:1.0')
    nu = 1.0
...
class _TwinFilterV2(NL_SMCMC_MLSWE_Filter_V2):
    """V2 filter with h(x)=arctan(x) obs-operator and Student-t (Cauchy) likelihood."""
    ...
    def __init__(self, isim, params):
        ...
        params = dict(params)
        params['obs_operator_name'] = f'arctan_studentt:{nu}'
        super().__init__(isim, params,
                         obs_operator=obs_operator_arctan)


def main():
    config_file = (sys.argv[1] if len(sys.argv) > 1
                   else os.path.join(_HERE, 'example_input_nldata_cauchy_v2.yml'))
