...
2. At each cycle, generate synthetic observations:
       y = arctan(H · z_truth) + ε,     ε ~ σ · t(ν=1)
   where arctan is a nonlinear operator
   and the noise follows a Cauchy (Student-t, ν=1) distribution.
...
The V2 variant (``run_nldata_cauchy_v2.py``) uses the same Student-t
likelihood but with per-block halo-localized MCMC instead of
full-domain parallel chains.

Usage
-----
    python3 -u nldata_synth_ob_cauchy/run_nldata_cauchy_v1.py [config.yml]

Default config: ``nldata_synth_ob_cauchy/example_input_nldata_cauchy_v1.yml``
"""
import os
import sys
import time
...
class _TwinFilter(_OrigFilter):
    """NL filter with h(x)=arctan(x) obs-operator and Student-t (Cauchy) likelihood.
...
def generate_truth_and_obs(params, H_b, bc_handler, tstart,
    ...
    synth_obs_file = os.path.join(outdir, 'synthetic_cauchy_obs.nc')
...
def main():
    config_file = (sys.argv[1] if len(sys.argv) > 1
                                     ...
                                     'example_input_nldata_cauchy_v1.yml'))
    with open(config_file) as f:
        params = yaml.safe_load(f)

    print("=" * 64)
    print("  NL Twin Experiment — arctan obs + Cauchy noise")
    ...
    params['obs_operator_name'] = 'arctan_studentt'

    print(f"\n{'='*64}")
    print(f"  Running NL-LSMCMC V1 on synthetic arctan+Cauchy obs")
    ...
    if M <= 1:
        ...
        print(f"\n{'='*64}")
        print(f"  NL twin (arctan+Cauchy) V1 complete in {elapsed:.1f}s "
              f"({elapsed/60:.1f} min)")
        ...
        print(f"\n{'='*64}")
        print(f"  NL twin (arctan+Cauchy) complete: M={M} runs averaged")
