#!/usr/bin/env python
"""
run_nldata_cauchy_letkf.py
===========================
Multiprocessing-parallelised LETKF baseline for the Cauchy noise twin experiment.
...
Uses:
    ...
    * **Gaussian likelihood** (misspecified — truth is Cauchy noise)
    * Pre-generated nature run + synthetic obs from V1 output directory

Usage
-----
    python3 nldata_synth_ob_cauchy/run_nldata_cauchy_letkf.py [config.yml]

Expects:
    - Nature run in ``output_nlgamma_twin_V1/truth_trajectory.nc``
    - Synthetic obs in ``output_nlgamma_twin_V1/synthetic_cauchy_obs.nc``
  (generate with:  python nldata_synth_ob_cauchy/run_nldata_cauchy_v1.py --truth-only)
"""
import os
import sys
import time
