#!/usr/bin/env python
"""
run_nongauss_twin_letkf.py
===========================
Multiprocessing-parallelised LETKF for the non-Gaussian twin experiment.
...
``run_nongauss_twin.py``.  The LETKF uses **linear** ``H @ x`` (no arctan)
and assumes Gaussian observation noise (misspecified — the true noise is a
3-component Gaussian mixture).

Usage
-----
    python3 nongauss_ldata/run_nongauss_twin_letkf.py [config.yml]

Expects:
    - Nature run in ``output_nongauss_twin_V1/truth_trajectory.nc``
    - Synthetic obs in ``output_nongauss_twin_V1/synthetic_nongauss_obs.nc``
  (generate with:  python nongauss_ldata/run_nongauss_twin.py --truth-only)
"""
import os
import sys
import time
...
# ===================================================================
#  LINEAR LETKF update (standard — no arctan)
# ===================================================================
def letkf_update_linear_mpi(xens, obs_values, obs_indices,
                            obs_errvar, covlocal_local,
                            nfields, ncells, nstart, nend):
    """
    Standard LETKF update with **linear** obs operator H @ x.
    """
    nanals = xens.shape[0]
    nobs = len(obs_values)
    ...
    # LINEAR obs operator: h(x) = x[obs_indices]
...
def _load_obs_netcdf(obs_file):
    """Read synthetic observation NetCDF."""
...
def _da_chunk_worker(args):
    ...
    # Linear LETKF update
    xens_local = letkf_update_linear_mpi(
...
def main():
    print("=" * 64)
    print("  Non-Gaussian Twin — LETKF (Gaussian assumption, multiprocessing)")
    print("=" * 64)
    ...
    print(f"[NG-LETKF] Grid: {nx}x{ny}, dimx={dimx}")
    print(f"[NG-LETKF] K={nanals}, loc={hcovlocal_scale_km}km, "
          f"inflate={covinflate1}")
    print(f"[NG-LETKF] nassim={nassim}, ncores={ncores}")

    # ---- Bathymetry ----
    from run_mlswe_lsmcmc_ldata_V1 import load_bathymetry
    ...
    if ssh_relax_rate > 0:
        try:
            ...
            print(f"[NG-LETKF] WARNING: SSH ref failed: {e}")
    ...
    h0, u0, v0, T0 = init_from_bc_handler(
    ...
    twin_dir = params.get('twin_dir', './output_nongauss_twin_V1')
    obs_file = os.path.join(twin_dir, 'synthetic_nongauss_obs.nc')
    truth_file = os.path.join(twin_dir, 'truth_trajectory.nc')

    if not os.path.exists(obs_file):
        print(f"ERROR: Synthetic obs not found: {obs_file}")
        print("Run:  python nongauss_ldata/run_nongauss_twin.py "
              "--truth-only")
        sys.exit(1)

    print(f"[NG-LETKF] Obs: {obs_file}")
    print(f"[NG-LETKF] Nature run: {truth_file}")
    ...
    print(f"[NG-LETKF] Obs/cycle: min={min(nobs_per)}, "
          f"max={max(nobs_per)}, mean={np.mean(nobs_per):.0f}")

    # ---- Output ----
    outdir = params.get('letkf_dir', './output_nongauss_twin_letkf')
    ...
    print(f"[NG-LETKF] Forecast pool: {n_fcst_workers} workers")
    ...
    print(f"[NG-LETKF] DA pool: {n_da_workers} workers")
    ...
    print(f"[NG-LETKF] Ensemble: {nanals} members initialised")
    ...
    #  CYCLING
    # =================================================================
    print(f"\n[NG-LETKF] Starting {nassim} cycles ...")
    ...
    for ntime in range(nassim):
        ...
        # ---- Linear LETKF update (mp.Pool with shared-memory pvens) ----
        t_da = time.time()
        if nobs > 0:
    ...
    with Dataset(outfn, 'w', format='NETCDF4') as nc_out:
        nc_out.createDimension('time', nassim + 1)
        nc_out.createDimension('layer', N_LAYERS)
        nc_out.createDimension('field', FIELDS_PER_LAYER)
        nc_out.createDimension('y', ny)
        nc_out.createDimension('x', nx)
        nc_out.createDimension('cycle', nassim)

        v = nc_out.createVariable('lsmcmc_mean', 'f4',
        ...
        rv = nc_out.createVariable('rmse_vel', 'f4', ('cycle',))
        rv[:] = rmse_vel_store.astype(np.float32)
        rs = nc_out.createVariable('rmse_sst', 'f4', ('cycle',))
        rs[:] = rmse_sst_store.astype(np.float32)
        rh = nc_out.createVariable('rmse_ssh', 'f4', ('cycle',))
        rh[:] = rmse_ssh_store.astype(np.float32)

        if H_b is not None:
            vb = nc_out.createVariable('H_b', 'f4', ('y', 'x'),
                                       zlib=True)
            vb[:] = H_b.astype(np.float32)

        vt = nc_out.createVariable('obs_times', 'f8', ('time',))
        if obs_times is not None and len(obs_times) >= nassim:
            vt[0] = tstart
            vt[1:] = obs_times[:nassim]

        nc_out.description = ('Non-Gaussian twin — LETKF '
                              '(Gaussian assumption, linear obs, multiprocessing)')
        nc_out.nanals = nanals
        nc_out.hcovlocal_scale_km = hcovlocal_scale_km
        nc_out.covinflate1 = covinflate1

    print(f"\n[NG-LETKF] Wrote {outfn}")
    print(f"[NG-LETKF] Mean RMSE_vel = "
          f"{np.nanmean(rmse_vel_store):.5f}")
    print(f"[NG-LETKF] Mean RMSE_sst = "
          f"{np.nanmean(rmse_sst_store):.4f}")
    print(f"[NG-LETKF] Mean RMSE_ssh = "
          f"{np.nanmean(rmse_ssh_store):.4f}")
    print("[NG-LETKF] Done!")
