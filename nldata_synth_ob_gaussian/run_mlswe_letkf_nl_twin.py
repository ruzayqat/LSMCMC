#!/usr/bin/env python
"""
run_mlswe_letkf_nl_twin.py
===========================
Multiprocessing-parallelised LETKF for the 3-layer MLSWE model with
**nonlinear** arctan observation operator, run as a synthetic twin experiment.

Uses ``multiprocessing.Pool`` for both forecast ensemble advancement and
LETKF analysis update — no MPI dependency.

Uses the same nature run trajectory and synthetic observations generated by
``run_mlswe_lsmcmc_nldata_V1_twin.py``.  The LETKF update applies
``h(x) = arctan(H @ x)`` instead of the linear ``H @ x``.

Usage
-----
    python3 run_mlswe_letkf_nl_twin.py [config.yml]

Expects:
    - Nature run trajectory in ``output_lsmcmc_nldata_V1/truth_trajectory.nc``
    - Synthetic obs in ``output_lsmcmc_nldata_V1/synthetic_arctan_obs.nc``
  (generate with:  python run_mlswe_lsmcmc_nldata_V1_twin.py --truth-only)
"""
import os
import sys
import time
...
# ===================================================================
#  Nonlinear LETKF update (arctan obs operator) - single cell chunk
# ===================================================================
def _letkf_update_chunk(xens, obs_values, obs_indices,
                        obs_errvar, covlocal_local,
                        nfields, ncells, nstart, nend):
    """
    LETKF analysis update with nonlinear arctan obs operator
    for grid cells nstart..nend (inclusive).
    """
    nanals = xens.shape[0]
    nobs = len(obs_values)
    ...
    # ---- Nonlinear obs operator: h(x) = arctan(H @ x) ----
...
def _load_obs_netcdf(obs_file):
    """Read observation NetCDF (synthetic arctan obs)."""
...
def _load_truth(truth_file, nassim, dimx):
    """Load nature run trajectory from NetCDF."""
...
# ===================================================================
#  Forecast pool  (shallow-copy pattern from lsmcmc_nl_V1.py)
# ===================================================================
_fcst_template = None       # pre-built MLSWE; fast-copied per worker
...
# ===================================================================
_da_pvens_buf = None        # mp.RawArray - parent writes, workers read
_da_nanals = None
_da_dimx = None
_da_nfields = None
_da_ncells = None
...
def _da_chunk_worker(args):
    ...
    # Read ensemble from shared memory (zero-copy numpy view)
    ...
    # Nonlinear LETKF update
    xens_local = _letkf_update_chunk(
        pvens, obs_values, obs_indices, obs_errvar,
        covlocal_local, _da_nfields, _da_ncells, nstart, nend)
...
def main():
    print("=" * 64)
    print("  NL-LETKF Twin Experiment (arctan obs, multiprocessing)")
    print("=" * 64)
    ...
    ncores = int(params.get('ncores', 25))

    print(f"[NL-LETKF] assimilate='{assimilate_fields}' "
          f"-> uv={assim_uv}, ssh={assim_ssh}, sst={assim_sst}")
    print(f"[NL-LETKF] Noise: h0={sig_x_ssh:.4f}, "
          f"uv={sig_x_uv:.4f}, T0={sig_x_sst:.4f}")
    print(f"[NL-LETKF] Grid: {nx}x{ny}, dimx={dimx}")
    print(f"[NL-LETKF] K={nanals}, loc={hcovlocal_scale_km}km, "
          f"inflate={covinflate1}")
    print(f"[NL-LETKF] nassim={nassim}, ncores={ncores}")

    # ---- Bathymetry ----
    from run_mlswe_letkf_nldata import load_bathymetry
    H_b = load_bathymetry(params, ny, nx, lon_grid, lat_grid)
    if H_b is not None:
        print(f"[NL-LETKF] Bathymetry: [{H_b.min():.0f}, {H_b.max():.0f}] m")
    ...
    if sst_nudging_rate > 0:
        ...
        if sst_files:
            ...
            print(f"[NL-LETKF] SST nudging: ref shape={sst_ref.shape}")
    ...
    if ssh_relax_rate > 0:
        try:
            ...
            print(f"[NL-LETKF] SSH relax: ref shape={ssh_ref_3d.shape}")
        except Exception as e:
            print(f"[NL-LETKF] WARNING: SSH ref failed: {e}")
    ...
    from run_mlswe_letkf_nldata import _make_init_state
    ...
    twin_dir = params.get('twin_dir', './output_lsmcmc_nldata_V1')
    obs_file = os.path.join(twin_dir, 'synthetic_arctan_obs.nc')
    truth_file = os.path.join(twin_dir, 'truth_trajectory.nc')

    if not os.path.exists(obs_file):
        print(f"ERROR: Synthetic obs not found: {obs_file}")
        print("Run:  python run_mlswe_lsmcmc_nldata_V1_twin.py --truth-only")
        sys.exit(1)

    print(f"[NL-LETKF] Obs: {obs_file}")
    print(f"[NL-LETKF] Nature Run: {truth_file}")

    yobs_list, ind_list, sig_list, obs_times = _load_obs_netcdf(obs_file)
    ...
    print(f"[NL-LETKF] Obs/cycle: min={min(nobs_per)}, "
          f"max={max(nobs_per)}, mean={np.mean(nobs_per):.0f}")

    # ---- Output directory ----
    outdir = params.get('letkf_dir', './output_letkf_nl_twin')
    ...
    print(f"[NL-LETKF] Forecast pool: {n_fcst_workers} workers")
    ...
    print(f"[NL-LETKF] DA pool: {n_da_workers} workers")

    # ---- Initialise ensemble (one MLSWE per member for IC only) ----
    ...
    print(f"[NL-LETKF] Ensemble: {nanals} members initialised")
    ...
    #  CYCLING
    # =================================================================
    print(f"\n[NL-LETKF] Starting {nassim} cycles ...")
    ...
    for ntime in range(nassim):
        ...
        # ---- NL-LETKF update (mp.Pool with shared-memory pvens) ----
        t_da = time.time()
        if nobs > 0:
    ...
    with Dataset(outfn, 'w', format='NETCDF4') as nc:
        ...
        nc.description = 'NL-LETKF twin experiment (arctan obs)'
        nc.nanals = nanals
        nc.hcovlocal_scale_km = hcovlocal_scale_km
        nc.covinflate1 = covinflate1

    print(f"\n[NL-LETKF] Wrote {outfn}")
    print(f"[NL-LETKF] Mean RMSE_vel = {np.nanmean(rmse_vel_store):.5f}")
    print(f"[NL-LETKF] Mean RMSE_sst = {np.nanmean(rmse_sst_store):.4f}")
    print(f"[NL-LETKF] Mean RMSE_ssh = {np.nanmean(rmse_ssh_store):.4f}")
    print("[NL-LETKF] Done!")
    ...
    try:
        ...
        for fname in ['truth_trajectory.nc', 'synthetic_arctan_obs.nc']:
        ...
        print("\n[runner] Generating NL-LETKF twin diagnostic plots ...")
        plot_nl_results(
            outdir=outdir,
            save_prefix='mlswe_results',
            config_file=None,
            method_label='NL-LETKF',
        )
    except Exception as e:
